...
func (manager *DockerManager) RunContainer(ctx context.Context, imageName string, env []string, pull bool, pipelineId string, operatorId string) (string, error) {
	...
	logging.Logger.Debug(fmt.Sprintf("Try to create docker container %s", imageName))
	...
	if err != nil {
		logging.Logger.Debug(fmt.Sprintf("Could not create container: %s", err.Error()))
		return "", err
	}
	logging.Logger.Debug(fmt.Sprintf("Try to start docker container with ID: %s", resp.ID))
